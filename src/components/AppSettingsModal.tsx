      ...
      >
        ...
        >
          ...
          <>
              ...
              <div style={row}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  {["Annual", "Standard", "Parts-Only"].map((o) => (
                    <button
                      key={o}
                      onClick={() => setSettings((s) => ({ ...s, gc_default: o }))}
                      style={{
                        flex: 1, padding: "6px 0", fontFamily: mono, fontSize: 11,
                        border: `1px solid ${settings.gc_default === o ? C.accent : C.inputBdr}`,
                        background: settings.gc_default === o ? "rgba(59,130,246,0.15)" : C.inputBg,
                        color: settings.gc_default === o ? C.accent : C.secondary,
                  ...
                  <div style={{ flex: "0 0 140px", padding: "10px 12px", borderRight: `1px solid ${C.border}` }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                      {[12, 24, 36, 48, 60].map((d) => (
                        <button
                          key={d}
                          onClick={() => setSettings((s) => ({ ...s, annual_duration: d }))}
                          style={{
                            padding: "5px 0", fontFamily: mono, fontSize: 10,
                            border: `1px solid ${settings.annual_duration === d ? C.accent : C.inputBdr}`,
                            background: settings.annual_duration === d ? "rgba(59,130,246,0.15)" : C.inputBg,
                            color: settings.annual_duration === d ? C.accent : C.secondary,
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "auto 1fr", columnGap: 6, rowGap: 2 }}>
                          {[["Start", d.start], ["Last", d.last], ["Duration", d.duration]].map(([k, v]) => (
                            <React.Fragment key={k}>
                              ...
                              <span style={{ color: C.secondary, fontSize: 10 }}>{v}</span>
                            </React.Fragment>
                          ))}
                        </div>
                      );
              ...
              {/* Service Condition Column visibility */}
              <div style={row}>
                <label style={labelStyle}>Service Condition Column</label>
                <div style={{ display: "flex", gap: 8 }}>
                  {[false, true].map((val) => (
                    <button
                      key={String(val)}
                      onClick={() => setSettings((s) => ({ ...s, show_svc_column: val }))}
                      style={{
                        flex: 1, padding: "6px 0", fontFamily: mono, fontSize: 11,
                        border: `1px solid ${settings.show_svc_column === val ? C.accent : C.inputBdr}`,
                        background: settings.show_svc_column === val ? "rgba(59,130,246,0.15)" : C.inputBg,
                        color: settings.show_svc_column === val ? C.accent : C.secondary,
                        borderRadius: 4, cursor: "pointer",
                      }}
                    >
                      {val ? "Visible" : "Hidden"}
